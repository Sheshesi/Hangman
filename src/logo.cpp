...
void Logo::ShowLogo(RenderWindow& window)
{
    ...
    titleSprite.setOrigin(
            ...
            fRect.top + fRect.height / 2.0f); //Определение центра спрайта
    titleSprite.setPosition(
            window.getSize().x / 2,
            window.getSize().y / 2); //Выставление спрайта по центру экрана
    ...
    while (isShowLogo) {
        ...
        float time
                = clock.getElapsedTime().asMicroseconds(); //Определение времени
                                                           //как микросекунды
